...
//------------- GET Data Customer -------------------------
//------------- GET Pagination Data Pasien Sorted -------------------------
function getPaginationPasienPerCustomerSortedAPI(biodataID, keyword, limit, page, sorter, ascdesc) {
	console.log(biodataID + ", " + keyword + ", " + limit + ", " + page + ", " + sorter + ", " + ascdesc);
	return $.ajax({
		"url": "/api/pasien/sortByPaginationPasienPerCustomer?biodata_id=" + biodataID + "&keyword=" + keyword + "&limit=" + limit + "&page=" + page + "&sorter=" + sorter + "&ascdesc=" + ascdesc,
		"method": "GET",
		"async": false
	});
}
...
function refreshList(keyword, page, limit, sorter, ascdesc) {

	if (keyword == null) {
		keyword = "";
		$("#btnResetFilter").attr("hidden", true);
	} else {
		$("#btnResetFilter").attr("hidden", false);
	}
	...
	console.log("Sorted By: " + sorter + " " + ascdesc);
	...
	var responsePasienPaginationSortBy = getPaginationPasienPerCustomerSortedAPI(parentBiodataID, keyword, limit, page, sorter, ascdesc).responseJSON;
	var dataPasienPaginationSortBy = responsePasienPaginationSortBy.data;
	var listPasien = dataPasienPaginationSortBy.data;
	console.log(dataPasienPaginationSortBy);
	console.log(dataPasienPaginationSortBy.total_data);
	...
	$("#banyakData").html(`Menampilkan ${minData} - ${(minData + parseInt(dataPasienPaginationSortBy.item_per_page)) - 1} dari ${dataPasienPaginationSortBy.total_data}`);
	...
	var totalPage = Math.ceil(dataPasienPaginationSortBy.total_data / limit);

	var prevPage = (parseInt(page) - 1);
	var nextPage = (parseInt(page) + 1);
	console.log("Halaman sebelumnya: " + prevPage + ", Halaman selanjutnya: " + nextPage);

	$('#btnSebelumnya').attr("onclick", "refreshList('" + keyword + "' , '" + prevPage + "' , '" + limit + "' , '" + sorter + "' , '" + ascdesc + "')");
	$('#btnSelanjutnya').attr("onclick", "refreshList('" + keyword + "' , '" + nextPage + "' , '" + limit + "' , '" + sorter + "' , '" + ascdesc + "')");
